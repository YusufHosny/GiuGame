...
TileViewText::TileViewText(int x, int y, float luminance, QGraphicsItem *parent) : ItemViewText(parent), luminance(luminance), x(x*50), y(y*50)  {}

void TileViewText::draw(std::shared_ptr<const GameObject> go) {
...
QRectF TileViewText::boundingRect() const {
...
void TileViewText::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *) {
    ...
    painter->drawText(-10,-10, 20, 20, Qt::AlignCenter, lumChar);
}
